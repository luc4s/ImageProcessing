...
//	static float boardSize = 380.f; // large Duplo board
	...
	public PVector get3DRotations(List<PVector> points2D) {
		if(!(points2D.size() == 4)) return new PVector(0, 0, 0);

		// 1- Solve the extrinsic matrix from the projected 2D points
		double[][] E = solveExtrinsicMatrix(sortCorners(points2D));
	...
	public static class CWComparator implements Comparator<PVector> {
		PVector center;
	...
	public static List<PVector> sortCorners(List<PVector> quad) {
		...
		for(int i=0;i < quad.size();i++){
			distance = quad.get(i).dist(origin);
			if(distance < minValue){
				min = i;
				minValue = distance;
			}
		}
//		PVector[] buffer = new PVector[quad.size()];
//		for(int i=min, j = 0;j < quad.size();i++){
//			buffer[j] = quad.get((i + quad.size()) % quad.size());
//			j++;
//		}
//		
//		for(int i=0;i < quad.size();i++){
